...
interface IInfinifiGateway {
    function getAddress(string memory tokenName) external view returns (address);
}
...
contract InfinifiPreviewer {
    ...
    function previewWithdrawToAsset(
        address gateway,
        uint256 assetAmount
    ) external view returns (uint256 iUSDAmount, uint256 siUSDAmount) {
        address redeemController = IInfinifiGateway(gateway).getAddress('redeemController');

        address accounting = IInfinifiRedeemController(redeemController).accounting();
        address assetToken = IInfinifiRedeemController(redeemController).assetToken();
        address receiptToken = IInfinifiRedeemController(redeemController).receiptToken();

        uint256 convertRatio = _getReceiptToAssetConvertRatio(accounting, assetToken, receiptToken);
        iUSDAmount = _convertAssetToReceipt(assetAmount, convertRatio);

        siUSDAmount = _receiptToStaked(IInfinifiGateway(gateway), iUSDAmount);
    }
    ...
    function _receiptToStaked(IInfinifiGateway _gateway, uint256 _amountIn) internal view returns (uint256) {
        ...
        address stakedToken = IInfinifiGateway(_gateway).getAddress("stakedToken");
        address yieldSharing = _gateway.getAddress("yieldSharing");
        
        uint256 totalAssets = IERC4626PreviewDeposit(stakedToken).totalAssets() + IYieldSharing(yieldSharing).vested();
        uint256 totalSupply = IERC4626PreviewDeposit(stakedToken).totalSupply();

        return _amountIn.mulDivDown(totalSupply + 1, totalAssets + 1);
    }
}
