...
type UsePercentSlippageInputOptions = {
  storageKey: string
  initial: string
  fallbackBps: number
}

export function parsePercentToBps(value: unknown, fallbackBps: number) {
  ...
  if (!Number.isFinite(testParse)) return fallbackBps
  ...
  if (!Number.isFinite(bps) || bps < 0) bps = fallbackBps
  const BPS_MAX = 10000 // 100% in basis points
  if (bps > BPS_MAX) bps = BPS_MAX
  return bps
}

export function usePercentSlippageInput({
  storageKey,
  initial,
  fallbackBps,
}: UsePercentSlippageInputOptions) {
  const [value, setValue] = useLocalStorage<string>(storageKey, initial)

  const valueBps = useMemo(() => parsePercentToBps(value, fallbackBps), [value, fallbackBps])

  return { value, setValue, valueBps }
}
