...
export function useFlashLoanAdjustment(tokenAddress: string, initial: string = '0.01') {
  const [flashLoanAdjustment, setFlashLoanAdjustment] = useLocalStorage<string>(
    `flash-loan-adjustment-${tokenAddress}`,
    initial,
  )

  const flashLoanAdjustmentBps = useMemo(() => {
    // Accept common user inputs like "2" or "2%"; trim whitespace and a trailing % sign
    const raw = (flashLoanAdjustment ?? '0') as unknown as string
  ...
  return { flashLoanAdjustment, setFlashLoanAdjustment, flashLoanAdjustmentBps }
}
