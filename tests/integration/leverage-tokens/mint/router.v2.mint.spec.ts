...
import { type Address, erc20Abi, parseUnits, encodeFunctionData } from "viem";
...
async function lifiQuote({
  ...
  if (!qRes.ok)
    throw new Error(`LiFi quote failed: ${qRes.status} ${qRes.statusText}`);
  ...
  if (!txRes.ok)
    throw new Error(
      `LiFi tx build failed: ${txRes.status} ${txRes.statusText}`,
    );
  ...
  const out = BigInt(
    step?.estimate?.toAmount ?? step?.estimate?.toAmountMin ?? "0",
  );
...
describe("Leverage Router V2 Mint (Tenderly VNet)", () => {
  ...
  it("mints shares successfully (happy path)", async () =>
    withFork(async ({ account, publicClient, walletClient }) => {
      ...
      const token: Address =
        (process.env.TEST_LEVERAGE_TOKEN as Address) ||
        (ADDR.leverageToken as Address);
      const manager: Address =
        (process.env.TEST_MANAGER as Address) || ADDR.manager;
      const router: Address =
        (process.env.TEST_ROUTER as Address) || ADDR.router;
      ...
      const { topUpNative, topUpErc20 } = await import(
        "../../../shared/funding"
      );
      ...
      if (debtQuote.out < neededFromDebtSwap) {
        ...
        debtIn =
          (debtIn * neededFromDebtSwap) /
          (debtQuote.out === 0n ? 1n : debtQuote.out);
      }
      const debtQuote2 =
      ...
      const maxSwapCost =
        (totalCollateral * BigInt(DEFAULT_MAX_SWAP_COST_BPS)) /
