...
import { useMintToken } from '@/features/leverage-tokens/hooks/useMintToken'
...
describe('useMintToken', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(simulateContract).mockResolvedValue({
      request: { address: tokenAddress, abi: [], functionName: 'mint' },
    } as any)
  ...
  describe('hook initialization', () => {
    it('should create a mutation with correct initial state', () => {
      const { result } = hookTestUtils.renderHookWithQuery(() => 
        useMintToken({ token: tokenAddress })
      )
  ...
  describe('successful mutation flow (simulate → write → wait)', () => {
    ...
    it('should return correct result after successful mint', async () => {
      ...
      const mutationResult = await result.current.mutateAsync(TOKEN_AMOUNT)
    ...
    it('should call onSuccess callback with transaction hash', async () => {
      ...
      const { result } = hookTestUtils.renderHookWithQuery(() => 
        useMintToken({ token: tokenAddress, onSuccess })
      )
  ...
  describe('error scenarios', () => {
    it('should throw error when wallet is not connected', async () => {
      ...
      await expect(result.current.mutateAsync(TOKEN_AMOUNT)).rejects.toThrow(
    ...
    it('should handle simulation errors', async () => {
      ...
      const onError = vi.fn()
      const { result } = hookTestUtils.renderHookWithQuery(() => 
        useMintToken({ token: tokenAddress, onError })
      )
