...
describe('orchestrateMint', () => {
  ...
  it('executes V2 flow with required quoteDebtToCollateral', async () => {
  ...
  it('throws error when quoteDebtToCollateral is missing', async () => {
    await expect(
      orchestrateMint({
        config: cfg,
        account,
        ...
        inputAsset,
        equityInInputAsset: 1000n,
        // quoteDebtToCollateral missing
      } as any),
    ).rejects.toThrow('quoteDebtToCollateral is required for router v2')
  })
})
